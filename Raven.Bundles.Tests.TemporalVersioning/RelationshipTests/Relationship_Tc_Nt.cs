...
{
    public class Relationship_Tc_Nt : RavenTestBase
    {
        [Fact]
        public void TemporalVersioning_Relationship_Tc_Nt()
        {
            using (var documentStore = this.GetTemporalDocumentStore())
            {
                documentStore.ExecuteIndex(new Employees_CurrentByDepartment());
                ...
                using (var session = documentStore.OpenSession())
                {
                    var results = session.Query<EmployeeWithDepartment, Employees_CurrentByDepartment>()
