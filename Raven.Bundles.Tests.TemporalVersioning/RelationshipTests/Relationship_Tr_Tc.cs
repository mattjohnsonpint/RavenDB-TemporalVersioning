...
{
    public class Relationship_Tr_Tc : RavenTestBase
    {
        [Fact]
        public void TemporalVersioning_Relationship_Tr_Tc()
        {
            using (var documentStore = this.GetTemporalDocumentStore())
            {
                documentStore.ExecuteIndex(new Employees_ByManagersCurrentName());
                ...
                using (var session = documentStore.OpenSession())
                {
                    var results = session.Effective(effectiveDate1)
                                         .Query<EmployeeWithManager, Employees_ByManagersCurrentName>()
                    ...
                    // we specifically asked for the managers CURRENT name
                    Assert.Null(results[0].Manager);
                    Assert.Equal("Alice Cooper", results[1].Manager);
