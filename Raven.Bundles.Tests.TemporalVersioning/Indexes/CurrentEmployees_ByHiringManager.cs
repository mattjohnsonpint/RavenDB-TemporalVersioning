...
namespace Raven.Bundles.Tests.TemporalVersioning.Indexes
{
    ...
    /// Current employees are indexed by their name and their hiring manager's name on the day they were hired.
    /// Complexity arises when its possible that employee had no hiring manager.
    /// </summary>
    public class CurrentEmployees_ByHiringManager : AbstractIndexCreationTask<Employee, EmployeeWithManager>
    {
        public CurrentEmployees_ByHiringManager()
        {
            Map = employees => from employee in employees

                               // filter the index to just current employees
                               ...
                               let employeeRevAtHiring = LoadDocument<Employee>(
                                   LoadDocument<TemporalHistory>(employee.Id + TemporalHistory.KeyExt)
                               ...
                               // If the employee had a manager, find the revision of that manager that was effective on the hire date.
